...
routes.post("/profile", async (req, res) => {
  const { username, email, fullName, country, city, address, phone, password } =
    req.body;
  try {
...
routes.put("/:id", async (req, res) => {
  ...
  try {
    const country = await countires.findByIdAndUpdate(
      id,
      {
        CountryName,
        TotalUser,
    ...
    if (!country) {
      return res
        .status(404)
        .json({ error: "No such country found or exists AH " });
    }
    res.status(200).json(country);
  } catch (error) {
    res.status(400).json({ error: error.message });
...
routes.post("/survey", async (req, res) => {
  ...
  try {
    const lastSurvey = await Survey.findOne({ username, country }).sort({
      createdAt: -1,
    });
...
routes.get("/survey", async (req, res) => {
  ...
  if (!username || !country) {
    return res
      .status(400)
      .json({ error: "Username and country are required." });
  }
  ...
  try {
    ...
    if (!survey) {
      return res
        .status(404)
        .json({
          error: "No survey found for this user in the specified country.",
        });
    }
